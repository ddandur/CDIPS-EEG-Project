...
OUTPUTdir = '../' + date + '_RFattempt/'
call('mkdir ' + OUTPUTdir, shell=True)
statuslog = open(OUTPUTdir + date + '_FirstRF.txt', 'a')
...
# Remove a random set of 3 subjects for use to validate later.
...
columnlabels = ['species', 'seizure', 'early', 'electrodeNum', 'meanCorr']

### Put the names of the features you'll be using to train here!
featurelabels = np.array(['species', 'meanCorr'])
...
traindf.to_csv('../' + date + '_RFattempt/IncludedDF.csv')
del dictdata
...
saveclf1 = OUTPUTdir + date + 'RFClassifierseizure.pkl'
...
# Save the classifier. Use joblib.load() when desired to retrieve.
saveclf2 = OUTPUTdir + date + 'RFClassifierEarly.pkl'
...
plt.savefig(OUTPUTdir + date + 'FeatureImportanceSeizure.png')
plt.clf()
plt.cla()
...
plt.savefig(OUTPUTdir + date + 'FeatureImportanceEarly.png')
plt.clf()
plt.cla()
...
valdf.to_csv('../' + date + '_RFattempt/ExcludedDF.csv')
del dictdata
...
testdf.to_csv('../' + date + '_RFattempt/TestDF.csv')
del dictdata
...
fileout = open(OUTPUTdir + date + '_Predictions.csv', 'a')
