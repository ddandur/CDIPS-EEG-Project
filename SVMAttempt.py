...
featurelabels = np.array(['meanVar', 'delta', 'theta', 'alpha', 'meanCorr', 'varVar', 'beta', 'lowGamma', 'highGamma'])
...
# Train classifier for predicting if seizure or not.
clf1 = svm.SVC(probability = True)
clf1.fit(traindf[featurelabels], traindf['seizure'])

# Save the classifier. Use joblib.load() when desired to retrieve.
saveclf1 = OUTPUTdir + 'SVClassifierseizure.pkl'
joblib.dump(clf1, saveclf1)

statuslog.write('Generated SVseizure:\n' + str(clf1) + '\n\n')

print 'Training SVC2...'
### If you want to try a different ML algorithm, change this part!
# Train classifier for predicting if early or not.
clf2 = svm.SVC(probability = True)
clf2.fit(traindf[featurelabels], traindf['early'])

# Save the classifier. Use joblib.load() when desired to retrieve.
saveclf2 = OUTPUTdir + 'SVClassifierEarly.pkl'
...
if (cutoff > 0):
	...
	print 'Finding ROC values...'
	valseizureprob = clf1.decision_function(valdf[featurelabels])
	valearlyprob = clf2.decision_function(valdf[featurelabels])
	# Then, we need to plug them into the ROC curve calculation.
	valsfpr, valstpr, _ = roc_curve(valdf['seizure'], valseizureprob[:])
	valefpr, valetpr, _ = roc_curve(valdf['early'], valearlyprob[:])
	# Plot and save.
